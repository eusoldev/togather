...
class SubAgentReport(models.TransientModel):
    _name = "sub.agent.report"
    _description = 'Sub Agent Wizard'
    ...
    def _print_report(self, data):
        ...
        if self.print_excel == 'pdf':
            return self.env.ref('sub_agent_report.report_for_sub_agent_report').report_action(self, data=data)
        if self.print_excel == 'excel':
            self.env.ref('sub_agent_report.report_for_sub_agent_report_excel').report_file = "Sub Agent Report %s"%(fields.Date.today())
            return self.env.ref('sub_agent_report.report_for_sub_agent_report_excel').report_action(self, data=data)
