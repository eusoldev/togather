...
class sub_agent_report(models.AbstractModel):
	_name = 'report.sub_agent_report.sub_agent_report'
	_description = 'Sub Agent Report'

	@api.model
	def _get_report_values(self, docids, data=None):
		record_wizard = self.env['sub.agent.report'].browse(self.env.context.get('active_ids'))
		...
		if typee == 'all':
			partner = self.env['res.partner'].search([('travel_agency','=',True)])
		else:
			partner = []
			for x in partner_id:
				partner.append(x)
		...
		for x in partner:
			...
			if payment_status == 'in_payment':
				payment_status_search = ('payment_state','in',['partial','not_paid'])
				record = self.env['account.move'].search([('date','>=',form),('date','<=',to),('partner_id','=',x.id),('state','=','posted'),('move_type','=','out_invoice'),payment_status_search])
			...
			for inv in record:
				...
				for route in package.flights_pkg:
					if route.from_loc and route.to_loc:
						service_route = route.from_loc.city + ' to ' + route.to_loc.city

				all_services = ''
				for ser in inv.invoice_line_ids:
					if ser.product_id.name:
						if not ser.product_id.name in all_services:
							all_services +=ser.product_id.name +', '
...
class sub_agent_report_excel(models.AbstractModel):
	_name = 'report.sub_agent_report.sub_agent_report.xslx'
	...
	def generate_xlsx_report(self, workbook, data, wizard_obj):
		...
		if agent_main_list:
			...
			for x in agent_main_list:
				...
				worksheet_agent_report.write(0, 5, 'Sub Agent Report', cell_format)
				...
				worksheet_agent_report.write(row, 12, 'Service Type', detailed_formate)
				worksheet_agent_report.write(row, 13, 'Route', detailed_formate)
				...
				for data in x['main_list']: 
					...
					worksheet_agent_report.write(line, 12, data['service'])
					worksheet_agent_report.write(line, 13, data['service_route'])
				...
				worksheet_agent_report.set_column(12, 12, 20)
				worksheet_agent_report.write(line, 12, '', detailed_formate)

				worksheet_agent_report.set_column(13, 13, 20)
				worksheet_agent_report.write(line, 13, '', detailed_formate)