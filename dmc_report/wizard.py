...
class SubAgentReport(models.TransientModel):
    ...
    def _print_report(self, data):
        ...
        if self.print_excel == 'pdf':
            return self.env.ref('dmc_report.report_dmc_report').report_action(self, data=data)
        if self.print_excel == 'excel':
            self.env.ref('dmc_report.report_for_dmc_report_xlsx').report_file = 'DMC Report %s'%str(fields.Date.today())
            return self.env.ref('dmc_report.report_for_dmc_report_xlsx').report_action(self, data=data)
