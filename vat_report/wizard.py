...
class VatReport(models.TransientModel):
    ...
    def generate_report(self):
        data = {}
        data['form'] = self.read(['form', 'to','company_id'])[0]
        # print(data)
        # data['object'] = self.
    ...
    def _print_report(self, data):
        data['form'].update(self.read(['form', 'to','company_id'])[0])
        if self.print_excel == 'pdf':
            return self.env.ref('vat_report.report_for_vat_report').report_action(self, data=data)
        if self.print_excel == 'excel':
            self.env.ref('vat_report.report_for_vat_report_xlsx').report_file = 'Vat Report %s'%str(fields.Date.today())
            return self.env.ref('vat_report.report_for_vat_report_xlsx').report_action(self, data=data)
