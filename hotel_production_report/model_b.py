	...
	def _get_report_values(self, docids, data=None):
		...
		if typee == 'all':
			...
			if report_type == 'destination':
				partner = self.env['res.country'].sudo().search([])
		else:
			partner = []
			if report_type == 'hotel':
				for x in partner_id:
		...
		for x in partner:
			...
			if report_type == 'vendor':
				...
				record = self.env['account.move'].sudo().search([('date','>=',form),('date','<=',to),hot_dest,('state','=','posted'),('type','=','in_invoice')])
			...
			if report_type == 'hotel':
				...
				record = self.env['account.move'].sudo().search([('date','>=',form),('date','<=',to),('invoice_origin','in',pkgs_list),('state','=','posted'),('type','=','in_invoice'),('hotel_ids','!=',False)])
			...
			for inv in record:
				...
				package = self.env['sale.order'].sudo().search([('name','=',inv.invoice_origin)])
				date_from = ""
				date_to = ""
				currency = ""
				nights = 0
													...
													if hotel.supplier.id == inv.partner_id.id:
														...
														if inv.amount_total:
															...
															main_list.append({
																...
																'status':dict(inv._fields['invoice_payment_state'].selection).get(inv.invoice_payment_state),
																})
															if same_suppliers==1:
										...
										for final in inv.hotel_ids:
											if final.id == hotel.hotel_id.id and final.id == x.id and hotel.hotel_id.id ==x.id:
												...
												if inv.amount_total:
													...
													# if report_type == 'hotel':
														# # if hotel.hotel_id in partner:
														# if inv.hotel_ids.ids == all_part_selected and hotel.hotel_id.id in all_part_selected:
														# # if inv.hotel_ids.ids in partner:

															# print(inv.hotel_ids.ids)

															# print(hotel.hotel_id.id)
															# print('2222222llllllllllll')
															# print(x.id)
...
class hotel_production_report_xlsx(models.AbstractModel):
	...
	def generate_xlsx_report(self, workbook, data, wizard_obj):
		...
		if hotel_summary_list:
			...
			worksheet_hotel_summery.write(11, 1, 'Hotel Production Report', cell_format)
			date_format = workbook.add_format({'align': 'centre', 'font_size': 13, 'underline': True})
			date = 'From: '+str(form)+' TO: '+ str(to)
			worksheet_hotel_summery.merge_range(11, 1,11, 4, 'Hotel Production Report',cell_format)
