...
export async function fetchValues(orm, model, field_name, value) {
    if (model && value) {
        ...
        return { [field_name]: records.length === 1 ? records[0] : false };
    }
    return { [field_name]: value };
}

export async function fetchCountryState(orm, model, country, state) {
    if (model && state) {
        let records = [];
        if(country && country != '' && country != undefined && country != null){
            records = await orm.searchRead(
                model,
                [['country_id', '=', country], '|', ['code', '=', state], ['name', 'ilike', state]],
                ['display_name'],
                { limit: 1 }
            );
        }
...
export function gmaps_get_geolocation(place, options) {
    if (!place) return {};
    const vals = {};
    Object.entries(options).forEach(([field, alias]) => {
        if (alias === 'latitude') {
...
export function gmaps_populate_places(place, place_options) {
    if (!place) return {};
    const values = {};
    Object.entries(place_options).forEach(([field, option]) => {
        if (Array.isArray(option) && !Object.prototype.hasOwnProperty.call(values, field)) {
            const vals = option.map(opt => place[opt]).filter(Boolean);
            values[field] = vals[0] || '';
        } else {
...
export function gmaps_populate_address(place, address_options, delimiter) {
    ...
    Object.keys(address_options).forEach(key => {
        fields_to_fill[key] = [];
    });

    Object.entries(address_options).forEach(([field, options]) => {
        options = [options].flat();
        const temp = {};
        place.address_components.forEach(component => {
            options.filter(opt => component.types.includes(opt)).forEach(match => {
                temp[match] = component[GOOGLE_PLACES_COMPONENT_FORM[match]] || false;
            });
        });
        fields_to_fill[field] = options.map(item => temp[item]);
    });

    Object.entries(fields_to_fill).forEach(([key, value]) => {
        const dlmter = fields_delimiter[key] || ' ';
        if (key === 'city') {
            result[key] = value.filter(Boolean)[0] || '';
        } else {
            result[key] = value.filter(Boolean).join(dlmter);
        }
    });
