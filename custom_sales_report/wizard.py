...
class SubAgentSalesReport(models.TransientModel):
    ...
    _description = 'Custom PDF Sale Report Wizard'

    form = fields.Date(string="From",required=True)
    to = fields.Date(string="To",required=True)
    print_excel = fields.Selection([
        ('excel', 'Excel Report'),
        ('pdf', 'PDF Report')], string='Report Format', help='Choose Report Format', default='pdf')
    ...
    booking_type = fields.Selection([
        ...
        ('book_date', 'Book Date')], string='Date Wise', default='arrival_date',required=True)

    # product_id = fields.Selection([
    #     ('flight', 'Flight'),
    #     ('hotel', 'Hotel'),
    #     ('both', 'Both')], string='Product', default='hotel')
    ...
    def generate_report(self):
        data = {}
        data['form'] = self.read(['form','to','partner_id','company_id','hotel','country_id','booking_type','product_id'])[0]
        return self._print_report(data)

    def _print_report(self, data):
        data['form'].update(self.read(['form','to','partner_id','company_id','hotel','country_id','booking_type','product_id'])[0])
        if self.print_excel == 'pdf':
            return self.env.ref('custom_sales_report.report_custom_sales_report').report_action(self, data=data)

        if self.print_excel == 'excel':
            self.env.ref('custom_sales_report.report_custom_sales_report_xlsx').report_file = 'Sales Report' + ' ' + str(fields.Date.today())
            return self.env.ref('custom_sales_report.report_custom_sales_report_xlsx').report_action(self, data=data)
