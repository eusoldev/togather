...
class quotation_builder(models.Model):
    ...
    reference = fields.Char(string="Ref No.", tracking=True) 
    ...
    def _compute_reservation_count(self):
        reservation_data = self.env['reservation.order'].read_group(
            [('quotation_builder_id', 'in', self.ids)],
            ['quotation_builder_id'],
            ['quotation_builder_id'],
        )
        count_by_qb = {
            data['quotation_builder_id'][0]: data['quotation_builder_id_count']
            for data in reservation_data
        }
        for rec in self:
            rec.reservation_count = count_by_qb.get(rec.id, 0)
    ...
    def action_view_related_reservations(self):
        ...
        return {
            ...
            'view_mode': 'list,form',
            'domain': [('quotation_builder_id', '=', self.id)],
            'context': {
                'default_quotation_builder_id': self.id,
                'default_partner_id': self.client_name.id,
                'default_name_package': self.subject or self.sr_no or self.reference,
            },
        }
...
‫- ‫جمیع‬ ‫الامكانیات‬ ‫والاسعار‬ ‫قابلة‬ ‫للتغییر"""
    ...
    def _prepare_reservation_common_line_vals(self, line, service_type):
        return {
            'service_type': service_type,
            'sequence': line.drag,
            'product_id': line.product_id.id,
            'supplier': line.supplier.id,
            'customer': self.client_name.id,
            'customer_m2m': [(6, 0, self.client_name.ids)],
            'no_of_person': line.no_of_person,
            'currency_name': line.currency_name.id or line.rate_hotel.id,
            'price': line.price,
            'amnt_fc': line.total,
            'exchange_rate': line.rate_hotel.inverse_rate,
            'commission': line.commission,
            'total': line.total,
            'description': line.description,
            'confirmation_no': line.confirmation_no,
            'service_tax_type': 'international',
        }

    def _clean_reservation_line_vals(self, vals):
        service_fields = self.env['all.services']._fields
        return {
            field_name: value
    ...
    def _prepare_reservation_hotel_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'hotel')
        vals.update({
            'hotel_id': line.resort_name.id,
            'date_from': line.check_in,
            'date_to': line.check_out,
            'nights': line.no_of_nights,
            'room_qty': line.number_of_rooms,
            'room_type': [(6, 0, line.room_type.ids)],
            'hotel_meal_plan': [(6, 0, line.meal_plane.ids)],
            'transfer': [(6, 0, line.transfer.ids)],
            'occasion': line.occasion,
            'special_remarks': line.special_remarks,
            'offered_applied': line.offers_appiled,
        })
        return self._clean_reservation_line_vals(vals)

    def _prepare_reservation_flight_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'flight')
        vals.update({
            'airline': line.airline.id,
            'from_loc': line.from_loc.id,
            'to_loc': line.to_loc.id,
            'date_from': line.departures.date() if line.departures else False,
            'date_to': line.arrival.date() if line.arrival else False,
            'flight': line.flight,
            'fight_class': line.fight_class.id,
    ...
    def _prepare_reservation_transfer_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'transfer')
        vals.update({
            'date_from': line.date_from,
            'date_to': line.date_to,
            'pick_up_place': line.pick_up_place,
            'drop_of_place': line.drop_of_place,
            'date': line.date,
            'transfer_type': line.transfer_type.id,
            'vehicle_type': line.vehicle_type.id,
            'wait_time_policy': line.wait_time_policy.id,
            'rent_car': line.rent_car,
        })
        return self._clean_reservation_line_vals(vals)

    def _prepare_reservation_tour_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'tours')
        vals.update({
            'date_from': line.ticket_day or line.date_from,
            'date_to': line.date_to,
            'guided_or_not': line.guided_or_not,
            'days_valid': line.days_valid.id,
            'offered_applied': line.offers_appiled,
        })
        return self._clean_reservation_line_vals(vals)

    def _prepare_reservation_visa_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'visa')
        vals.update({
            'date_from': line.date_from,
            'date_to': line.date_to,
            'visa_status': line.visa_status,
            'offered_applied': line.offers_appiled,
        })
        return self._clean_reservation_line_vals(vals)

    def _prepare_reservation_package_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'ready_package')
        vals.update({
            'validity': line.validity,
            'no_of_nights': line.no_of_nights,
            'rooms_type': line.rooms_type,
            'meal_plan': line.meal_plan.id,
    ...
    def _prepare_reservation_private_jet_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'private_jet')
        vals.update({
            'date_from': line.departures.date() if line.departures else False,
            'date_to': line.arrival.date() if line.arrival else False,
            'from_to': line.from_to,
            'flight': line.flight,
            'fight_class': line.fight_class.id,
            'departures': line.departures,
            'arrival': line.arrival,
    ...
    def _prepare_reservation_yacht_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'yacht')
        vals.update({
            'date_from': line.date_from,
            'date_to': line.date_to,
            'duration': line.duration,
            'yacht_type': line.yacht_type.id,
            'meal_plan': line.meal_plan.id,
            'occupancy': line.occupancy,
            'departure_place': line.departure_place,
            'arrival_place': line.arrival_place,
        })
        return self._clean_reservation_line_vals(vals)

    def _prepare_reservation_cruise_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'cruise')
        vals.update({
            'date_from': line.date_from,
            'date_to': line.date_to,
            'duration': line.duration,
            'cabin_type': line.cabin_type.id,
            'meal_plan': line.meal_plan.id,
            'occupancy': line.occupancy,
            'departure_place': line.departure_place,
            'arrival_place': line.arrival_place,
    ...
    def _prepare_reservation_other_line_vals(self, line):
        vals = self._prepare_reservation_common_line_vals(line, 'other')
        vals.update({
            'date_from': line.date_from,
            'date_to': line.date_to,
        })
    ...
    def _get_quotation_lines_by_service_type(self):
        service_map = {
            'hotel': ('hotel_pkg', self._prepare_reservation_hotel_line_vals),
            'flight': ('flights_pkg', self._prepare_reservation_flight_line_vals),
            'transfer': ('transportation_pkg', self._prepare_reservation_transfer_line_vals),
            'tour': ('tours_pkg', self._prepare_reservation_tour_line_vals),
            'visa': ('visa_pkg', self._prepare_reservation_visa_line_vals),
            'package': ('packages_pkg', self._prepare_reservation_package_line_vals),
            'private': ('privatejet_pkg', self._prepare_reservation_private_jet_line_vals),
            'yacht': ('yacht_pkg', self._prepare_reservation_yacht_line_vals),
            'cruise': ('cruises_pkg', self._prepare_reservation_cruise_line_vals),
            'other': ('otherservices_pkg', self._prepare_reservation_other_line_vals),
        }
        result = {
            reservation_field: []
            for reservation_field, prepare_method in service_map.values()
        }
        quotation_fields = (
            'tree_link_id',
            'flights_pkg',
        ...
        seen_line_ids = set()
        for quotation_field in quotation_fields:
            for line in getattr(self, quotation_field):
                if line.id in seen_line_ids:
                    continue
                seen_line_ids.add(line.id)
                service_info = service_map.get(line.service_type)
                if not service_info:
                    continue
                reservation_field, prepare_method = service_info
                result[reservation_field].append((0, 0, prepare_method(line)))
        return result

    def action_create_reservation(self):
        self.ensure_one()
        if not self.client_name:
            raise ValidationError(_('Please select Client Name before creating reservation.'))
        if not self.tree_link_id:
            raise ValidationError(_('Please add services details.'))
        reservation_vals = {
            'quotation_builder_id': self.id,
            'quote_builder': False,
            'package': True,
            'name_package': self.subject or self.sr_no or self.reference,
            'year': datetime.now().year,
            'partner_id': self.client_name.id,
            'no_of_particpents': self.number_of_guests,
            'destination': [(6, 0, self.destination.ids)],
            'arrival_date': self.date_from.date() if self.date_from else False,
            'departure_date': self.date_to.date() if self.date_to else False,
            'date_order': self.creation_date or fields.Datetime.now(),
            'customer_ref': self.reference,
            'validity_date': self.offers_validity_date,
            'hotels_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'flights_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'transfers_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'tours_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'visa_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'packages_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'private_jet_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'yacht_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'cruise_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
            'other_cancel_policy': self.cancel_new_eng or self.terms_conditions_eng,
        }
        reservation_vals.update(self._get_quotation_lines_by_service_type())
        ...
        service_lines = (
            ...
            reservation.transportation_pkg |
            reservation.tours_pkg |
            reservation.visa_pkg |
            reservation.packages_pkg |
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['reference'] = self.env['ir.sequence'].next_by_code('quotation.builder.seq')
        new_record = super(quotation_builder, self).create(vals_list)
...
class ReservationOrder(models.Model):
    ...
    quotation_builder_id = fields.Many2one(
        'quotation.builder',
        string='Quotation Builder',
        copy=False,
        index=True,
    )
